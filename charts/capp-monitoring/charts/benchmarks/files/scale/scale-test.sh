...
if [ -z "$VM_IMPORT_URL" ]; then
  echo "VM_IMPORT_URL not set — skipping metric push."
  exit 0
fi

echo "==> Pushing metrics to VictoriaMetrics ($VM_IMPORT_URL)..."
{
  ...
  "${VM_IMPORT_URL}/api/v1/import/prometheus?extra_label=capp=${KSVC_NAME}&extra_label=namespace=${KSVC_NAMESPACE}"
echo "    done."
